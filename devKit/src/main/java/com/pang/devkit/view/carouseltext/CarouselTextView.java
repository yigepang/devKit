package com.pang.devkit.view.carouseltext;
...
/**
 * Created by hoozy on 2022/4/13
 * 轮播textview
 */
public class CarouselTextView extends RelativeLayout {
    ...
    private ICarouselTextItemClickListener mListener;
     ...
     *
     */
    public void setDataWithTwoTextView(List<String> startTextData, List<String> endTextData) {
        this.mDatas = endTextData;
        if (DisplayUtils.isEmpty(mDatas)) {
            return;
        }
        mViewFlipper.removeAllViews();
        for (int i = 0; i < mDatas.size(); i++) {
            View inflate = LayoutInflater.from(mContext).inflate(R.layout.layout_text_banner_two_view, this, false);
            TextView tvFirst = inflate.findViewById(R.id.tv_first_page_work_condition);
            TextView tvSecond = inflate.findViewById(R.id.tv_well_num_date);
            tvFirst.setText(startTextData.get(i));
            tvSecond.setText(endTextData.get(i));
            mViewFlipper.addView(inflate, i);//添加子view,并标识子view位置
        }
    }
     ...
     * 设置点击监听事件回调
     */
    public void setItemOnClickListener(ICarouselTextItemClickListener listener) {
