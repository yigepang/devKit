...
public class CustomCalendarView extends FrameLayout implements BaseRecyclerAdapter.OnItemClickListener {
    ...
    public void onItemClick(Object o, int position) {
        CalendarBean o1 = (CalendarBean) o;
        if (null != o1.getDate()) {
            listeners.onClick(o1, position);
        }
    }

    private void initAdapter() {
        mAdapter = new BaseRecyclerAdapter<CalendarBean>(getContext(), mDatas, R.layout.layout_date_item) {
            TextView lastView;

            @Override
            public void onBindViewHolder(ViewHolder viewHolder, final CalendarBean itemVO, final int position) {
                final TextView tv_day = viewHolder.findViewById(R.id.tv_day);
                tv_day.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (null != lastView && tv_day != lastView) {
                            lastView.setBackgroundResource(0);
                            lastView.setTextColor(getResources().getColor(R.color.color_434343));
                        }
                        if (itemVO.isSelect()) {
                            tv_day.setBackgroundResource(0);
                            itemVO.setSelect(false);
                        } else {
                            tv_day.setBackgroundResource(R.drawable.bg_calendar_text_select_true);
                            itemVO.setSelect(true);
                        }
                        lastView = v.findViewById(R.id.tv_day);
                    }
                });
                tv_day.setText(itemVO.getDay());
                //0：当月，1：上月，2：下一个月
                if (itemVO.getDayType() == 1 || itemVO.getDayType() == 2) {
                    //上个月数据
    ...
    private onCalendarItemClickListener listeners;
    ...
    public interface onCalendarItemClickListener {
        void onClick(CalendarBean bean, int position);
    }
